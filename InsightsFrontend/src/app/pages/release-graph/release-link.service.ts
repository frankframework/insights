...
export class ReleaseLinkService {
	...
	createLinks(sortedGroups: Map<string, ReleaseNode[]>[]): ReleaseLink[] {
		...
		return [
			...this.createMasterBranchLinks(masterNodes),
			...this.createSubBranchLinks(sortedGroups.slice(1), masterNodes)
		];
	}

	private createMasterBranchLinks(nodes: ReleaseNode[]): ReleaseLink[] {
		const links: ReleaseLink[] = [];
		for (let i = 0; i < nodes.length - 1; i++) {
			links.push(this.buildLink(nodes[i], nodes[i + 1]));
		}
		return links;
	}

	private createSubBranchLinks(
			subGroups: Map<string, ReleaseNode[]>[],
		...
		for (const subGroup of sortedSubGroups) {
			...
			links.push(...this.createSequentialLinks(subNodes));
		}
	...
	private createSequentialLinks(nodes: ReleaseNode[]): ReleaseLink[] {
		const links: ReleaseLink[] = [];
		for (let i = 0; i < nodes.length - 1; i++) {
			links.push(this.buildLink(nodes[i], nodes[i + 1]));
		}
