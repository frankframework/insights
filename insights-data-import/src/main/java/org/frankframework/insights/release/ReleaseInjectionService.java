...
public class ReleaseInjectionService {
    ...
    /**
     * Constructor for ReleaseInjectionService.
     *
     * @param gitHubGraphQLClient Client for interacting with GitHub API.
     ...
     * @param branchInjectionService Service for injecting branches.
     ...
     */
    public ReleaseInjectionService(
    ...
    public void injectReleases() throws ReleaseInjectionException {
        try {
            Set<ReleaseDTO> releaseDTOs = gitHubGraphQLClient.getReleases();

            List<Branch> allBranches = branchInjectionService.getAllBranches();
            Map<String, Set<BranchPullRequest>> pullRequestsByBranch =
                    branchInjectionService.getBranchPullRequestsByBranches(allBranches);
    ...
    private void saveAllReleases(Set<Release> releases) {
        ...
        releases.forEach(release -> replaceOldScannedPropertyToNew(existingLastScanned, release));
    ...
    /**
     * Replaces the lastScanned property of a release with the existing value from the database if it exists.
     * This ensures that the lastScanned timestamp is preserved during updates.
     *
     * @param existingLastScanned Map of release IDs to their existing lastScanned values.
     * @param release The release to update.
     */
    private void replaceOldScannedPropertyToNew(Map<String, OffsetDateTime> existingLastScanned, Release release) {
