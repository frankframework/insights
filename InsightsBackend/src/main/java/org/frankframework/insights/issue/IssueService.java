...
public class IssueService {
	...
	public void injectIssues() throws IssueInjectionException {
		...
		try {
			...
			Map<String, IssueDTO> issueDtoMap = issueDTOS.stream()
					.collect(Collectors.toMap(IssueDTO::id, dto -> dto));
	...
	private Set<Issue> assignSubIssuesToIssues(List<Issue> issues, Map<String, IssueDTO> issueDtoMap) {
		Map<String, Issue> issueMap = issues.stream()
				.collect(Collectors.toMap(Issue::getId, dto -> dto));

		issues.forEach(issue -> {
			IssueDTO issueDTO = issueDtoMap.get(issue.getId());
			if (issueDTO != null && issueDTO.subIssues() != null && issueDTO.subIssues().getEdges() != null) {
