...
class ReleaseServiceTest {
    ...
    public void setUp() {
		...
		when(gitHubRepositoryStatisticsService.getGitHubRepositoryStatisticsDTO()).thenReturn(statisticsDTO);
    }
	...
	public void should_AssignPullRequestsToRelease() throws Exception {
		...
		ReleasePullRequest releasePR = mockRelease.getReleasePullRequests().iterator().next();
	...
	public void should_HandleMultipleReleasesOnSameBranch() throws Exception {
		...
		when(gitHubRepositoryStatisticsService.getGitHubRepositoryStatisticsDTO().getGitHubReleaseCount()).thenReturn(2);
		when(releaseRepository.count()).thenReturn(0L);
