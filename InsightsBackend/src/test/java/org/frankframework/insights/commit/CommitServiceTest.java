...
class CommitServiceTest {
	...
	void setUp() {
		...
		when(gitHubRepositoryStatisticsService.getGitHubRepositoryStatisticsDTO()).thenReturn(mockGitHubRepositoryStatisticsDTO);
	}

	@Test
	public void should_InjectCommitsForBranch_when_DatabaseIsNotFilledYet() throws GitHubClientException, CommitInjectionException, MappingException {
		when(mockGitHubRepositoryStatisticsDTO.getGitHubCommitCount(eq(Collections.emptyList()))).thenReturn(10);
	...
	public void should_ThrowCommitInjectionException_when_ErrorOccurs() throws GitHubClientException {
		...
		when(commitRepository.countAllBranchCommitRelations()).thenReturn(0);
		when(branchService.getAllBranches()).thenReturn(List.of(mockBranch));
		when(gitHubClient.getBranchCommits(mockBranch.getName())).thenThrow(new GitHubClientException("Error fetching commits", null));
