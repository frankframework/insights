...
public class PullRequestService {
	...
	private Set<PullRequestDTO> fetchCombinedPullRequestsForBranch(Branch branch) throws GitHubClientException {
		...
		log.info("Fetched {} pull requests for branch with current name: [{}]", currentPullRequests.size(), currentName);
		...
		if (matcher.matches()) {
			...
			log.info("Branch [{}] matches historical pattern. Querying for old name [{}] as well.", currentName, oldName);
		...
		log.info("Total unique pull requests for branch [{}] after combining historical data: {}", currentName, combinedPullRequests.size());
