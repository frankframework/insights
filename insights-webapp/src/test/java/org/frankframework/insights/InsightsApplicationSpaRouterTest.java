...
/**
 * Tests {@link InsightsApplication#spaRouter(String)}: which requests are answered with a file from the Angular build
 * output, which fall back to `index.html` so the Angular router can resolve them, and which are left alone entirely.
 *
 * <p>The router is pointed at `src/test/resources/spa-fixture/` instead of the real build output, which only exists
 * after `pnpm build` has run.
 *
 * <p>Regression coverage for <a href="https://github.com/frankframework/insights/issues/712">issue 712</a>: a release
 * branch nightly such as "10.2-nightly" used to 404, because the router decided what was a static resource by looking
 * at the path extension and "10.2-nightly" ends in a dot followed by a non-numeric suffix.
 */
public class InsightsApplicationSpaRouterTest {
    ...
    @Nested
    class ReleaseDetailRoutes {

        /**
         * Every tag name the Frank!Framework publishes has to reach the Angular router, no matter how many dots it
         * holds or what it is suffixed with.
         */
    ...
    class ClientSideRoutes {
        ...
        /** A directory is not a file, so requesting one is a client-side route as far as the router is concerned. */
        @ParameterizedTest(name = "{0}")
    ...
    class StaticResources {
        ...
        /** These exist in the fixture, so they have to be served as themselves rather than as the index. */
        @ParameterizedTest(name = "{0}")
        @ValueSource(
                strings = {
                    "/index.html",
        ...
        /** The counterpart of the bug: a file without any extension is still recognised as a file. */
        @Test
        ...
        /** A path that looks like an asset but is not in the build output is treated as a client-side route. */
        @ParameterizedTest(name = "{0}")
        ...
        @Test
        public void pathTraversal_isNotServed() {
    ...
    class ExcludedPaths {
        ...
        /** The API and the error page are handled elsewhere; falling back to HTML here would mask their 404s. */
        @ParameterizedTest(name = "{0}")
        @ValueSource(
                strings = {
                    "/api",
        ...
        /** `/api` is excluded as a path, not as a prefix of a segment. */
        @Test
        public void routeStartingWithApiLetters_isServedTheIndex() {
            assertThat(servedResource("/apidocs")).contains(INDEX);
        }

        /** `/error` is matched exactly, so a client-side route nested under it still reaches the SPA. */
        @Test
    ...
    /**
     * Pins down the behaviour that caused the bug. The suffix is what `UriUtils.extractFileExtension` used to hand to
     * the routing decision; none of these paths may be mistaken for a file any more.
     */
    @Nested
    class BugCharacterisation {
    ...
    /** The bean the application actually registers has to point at the real Angular build output. */
    @Test
    ...
    /**
     * Routes the request and returns the classpath location of the resource that would be written to the response, or
     * an empty optional when no route matches at all.
     */
